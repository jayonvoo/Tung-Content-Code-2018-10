...
(function () {
    ...
    function keyFunction() {
        ...
        //檢測在0.1秒内是否有一次以上的按鍵事件
        if (Timer == null) {
            firstKey = event.keyCode;

            Timer = setTimeout(ResetTimer, 100);
        } else {
            if (firstKey == 17) {
                if (event.keyCode == 51) {  //19寸電話_Keycode = 51
                    window.plugins.launcher.launch({ packageName: 'bc.pt.dd' }, successCallback, errorCallback);
                } else if (event.keyCode == 52) {  //19寸電話_Keycode = 52
                    window.plugins.launcher.launch({ packageName: 'org.doubango.imsdroid' }, successCallback, errorCallback);
                } else if (event.keyCode == 55) {  //19寸電話_Keycode = 17 + 55
                    window.plugins.launcher.launch({ packageName: 'ndt.mobile.android.turnoffscreen' }, successCallback, errorCallback);
                }
            }
        }
        ...
        if (event.keyCode == 119) {  //RII遙控器F8_Keycode = 119
            ...
            if (isAppOpen == true) {  //app開過之後回到首頁并且初始化focus坐標
    ...
    function BindApplicationModal(pCategory) {
        ...
        //The body should have at least 2 input fields outside of the dialog to trap focusing, otherwise focusing may be outside of the document and we will loss control in such a case.
        //So we create 2 dummy text fields with width = 0 and opacity = 0
        var dummy = "<input style='width:0; opacity:0'/>";
        var clickedOutside = false;
        $('body').append(dummy).prepend(dummy);
        $('#ModalImage').focusout(function (e) {
            if (clickedOutside) {
                e.target.focus();
